...
# bash bash/seq_norm/cg_norm_N_1000.sh
...
rm outputs/seq_norm/cg_norm_N_1000.txt
...
for m in ${M[@]}; do
	for n in ${N[@]}; do
		...
		then	
			...
			((line++))
			str="$(sed "${line}q;d" outputs/seq_norm/cg_norm_it_N_1000.txt)"
			...
			for (( i=0; i<loop_variable; i++ )) do
				export OMP_NUM_THREADS=1
				str=$(./bin/cg_eigen.exe dense_norm $m $n $it)
				IFS=' ' read -r -a arr <<< "$str"
				time=${arr[4]}
				error=${arr[2]}
				res=${arr[3]}
			...
			echo "$m $n $it $sum $avge $avgr" >> outputs/seq_norm/cg_norm_N_1000.txt
		fi
	done
done